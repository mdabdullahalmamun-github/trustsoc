...
MODEL = "llama3"
RESULTS_FILENAME = "ctibench_ate_results_llama3_k11.json"
...
def main():
    print("WARNING: k=11 exceeds the k>=10 threshold where LLaMA 3 has previously")
    print("crashed on this hardware (documented in Chapter 4.7 -- RTX 3050, 4GB VRAM,")
    print("CUDA out-of-memory). This run may fail partway through. If it does, your")
    print("existing k=5 results are unaffected and remain your safe, primary result.")
    print()
    ...
    results = []
    for it in items:
        answer = trustsoc_answer(it["question"], model=MODEL, k=11)
